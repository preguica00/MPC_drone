function cost = costfunction(y, H)
    ...
    cost = sum(2*(x(:)-x_final).^2+(z(:)-z_final).^2 + (theta(:)-theta_final).^2 + (x_velocity(:)-dotx_final).^2+ (z_velocity(:)-dotz_final).^2+(angular_velocity(:)-dottheta_final).^2 + (mode_diff(:)).^2+(mode_common(:)).^2);   
